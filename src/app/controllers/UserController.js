...
class UserController {
  async store(req, res) {
    ...
    const { id, name, email, provider } = await User.create(req.body); // substitui const {user} para não retornar tudo.
  ...
  async update(req, res) {
    const schema = Yup.object().shape({
      ...
      password: Yup.string().min(6).when('oldPassword', (oldPassword, field) => // field se refere a propriedade atual (password)
